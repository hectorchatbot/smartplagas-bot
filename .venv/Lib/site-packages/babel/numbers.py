...
def get_infinity_symbol(
    ...
    *,
    ...
    """
    locale = Locale.parse(locale or LC_NUMERIC)
    return _get_number_symbols(locale, numbering_system=numbering_system).get('infinity', 'âˆž')
...
def format_compact_currency(
    ...
    *,
    ...
    number, format = _get_compact_format(number, compact_format, locale, fraction_digits)
    # Did not find a format, fall back.
    if format is None or "Â¤" not in str(format):
...
class NumberPattern:
    ...
    def compute_scale(self) -> Literal[0, 2, 3]:
        ...
        if '%' in ''.join(self.prefix + self.suffix):
            scale = 2
        elif 'â€°' in ''.join(self.prefix + self.suffix):
            scale = 3
        return scale
    ...
    def apply(
        ...
        *,
        ...
        if 'Â¤' in retval and currency is not None:
            retval = retval.replace('Â¤Â¤Â¤', get_currency_name(currency, value, locale))
            retval = retval.replace('Â¤Â¤', currency.upper())
            retval = retval.replace('Â¤', get_currency_symbol(currency, locale))
